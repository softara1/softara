...
/* =====================================================================
   PREMIUM ENHANCEMENTS — Added by AI Upgrade
   All functions/variables here are NEW and do not modify existing ones.
   They are wrapped in IIFE to avoid polluting the global scope.
   ===================================================================== */
...
(function premiumEnhancements() {
    ...
    // Convert scroll, touch, wheel events to passive for better performance
    ...
    function setupSmoothScroll() {
        if (!('scrollBehavior' in document.documentElement.style)) {
            // Native smooth scroll not supported — polyfill
    ...
    function setupLazyAnimations() {
        if (!('IntersectionObserver' in window) || utils.prefersReducedMotion()) return;
        ...
        function observeElements() {
            document.querySelectorAll(`
                ...
                .topic-list-item
            `).forEach(el => {
                if (!el.classList.contains('in-view') && !el.dataset.observed) {
        ...
        const contentObserver = new MutationObserver(utils.debounce(observeElements, 300));
        contentObserver.observe(document.body, { childList: true, subtree: true });
    }

    /* ===================== 5. IMAGE LAZY LOADING POLYFILL ===================== */
    ...
    /* ===================== 6. MODAL ACCESSIBILITY IMPROVEMENTS ===================== */
    ...
    function setupMobileTouch() {
        ...
        document.addEventListener('touchstart', function(e) {
            const btn = e.target.closest('button, .btn-action, .btn-icon, .f-btn, .fab, .page-btn');
            if (btn) {
        ...
        document.addEventListener('touchend', function(e) {
            ...
            if (now - lastTouchEnd <= 300) {
                const btn = e.target.closest('button, a, .btn-action, .btn-icon');
                if (btn) e.preventDefault();
            }
            lastTouchEnd = now;
        }, { passive: false });

        // Better swipe gesture for closing modals on mobile
        let touchStartY = 0;
        let touchStartX = 0;
        document.addEventListener('touchstart', function(e) {
        ...
        document.addEventListener('touchmove', function(e) {
            ...
            if (modal && modal.id !== 'confirmDeleteModal') {
                ...
                // Swipe down to close (large enough movement)
    ...
    function setupPerformanceOptimizations() {
        ...
        // Add CSS to pause animations when tab is hidden
        const style = document.createElement('style');
        style.textContent = `
            body.tab-hidden * {
                animation-play-state: paused !important;
            }
            body.is-scrolling .flip-card,
            body.is-scrolling .cat-title i,
            body.is-scrolling .forum-float-btn i {
        ...
        // Detect scrolling and pause animations temporarily
        let scrollTimeout;
        ...
        if ('requestIdleCallback' in window) {
            window.requestIdleCallback(() => {
                ...
                const criticalLinks = ['/forum', '/latest'];
    ...
    /* ===================== 10. BETTER MODAL CLOSE HANDLING ===================== */
    function setupModalCloseHandling() {
        // Already partially handled by existing code, but add backdrop click handling
        document.addEventListener('click', function(e) {
            const modal = e.target.closest('.modal-overlay.active');
            if (!modal) return;
            // If clicked outside the modal-box, close it
            if (e.target === modal) {
                modal.classList.remove('active');
                if (modal.id === 'quickViewModal') {
                    modal.style.display = 'none';
    ...
    function setupConsoleLogo() {
        ...
        console.log('%c🚀 المكتبة الرقمية — النسخة المُطورة', styles);
        console.log('%cمُحسّن بأحدث التقنيات | متجاوب مع جميع الأجهزة | أداء عالٍ', 'color: #94a3b8; font-size: 11px;');
    }

    /* ===================== 12. ERROR HANDLING (global) ===================== */
    function setupErrorHandling() {
        // Catch uncaught errors gracefully
    ...
    function setupThemeSync() {
        ...
        window.addEventListener('storage', function(e) {
            if (e.key === 'zzone_theme' && e.newValue) {
                ...
                if (meta) {
                    meta.setAttribute('content', e.newValue === 'light' ? '#f1f5f9' : '#060912');
                }
            }
            if (e.key === 'zzone_favs') {
                // Refresh favorites state across tabs
                if (typeof window.executeSearchAndFilter === 'function') {
                    try { window.executeSearchAndFilter(); } catch(err) {}
                }
            }
        });
    }

    /* ===================== 14. VIEWPORT HEIGHT FIX (mobile) ===================== */
    ...
    /* ===================== 15. BETTER FORM VALIDATION ===================== */
    function setupFormValidation() {
        // Real-time validation for inputs
        document.addEventListener('input', function(e) {
            const input = e.target;
            if (!input.matches('input[type="url"], input[type="email"]')) return;
    ...
    /* ===================== 17. CSS VIEWPORT UNITS POLYFILL ===================== */
    function setupViewportUnits() {
        ...
        setViewportUnits();
        window.addEventListener('resize', utils.debounce(setViewportUnits, 150));
        window.addEventListener('orientationchange', () => setTimeout(setViewportUnits, 300));
    }
    ...
    /* ===================== 19. NETWORK-AWARE LOADING ===================== */
    function setupNetworkAware() {
        if (!navigator.connection) return;

        const connection = navigator.connection;
        function adjustForNetwork() {
            ...
            if (saveData || effectiveType === 'slow-2g' || effectiveType === '2g') {
                ...
                const style = document.createElement('style');
                style.textContent = `
                    .data-saver .forum-float-btn::after,
                    .data-saver .cat-title i,
    ...
    function setupRippleEffect() {
        ...
        // Use event delegation to add ripple to dynamically added buttons
        document.addEventListener('pointerdown', function(e) {
            const btn = e.target.closest('.btn-action, .btn-icon, .btn-main, .btn-sec, .tool-btn, .f-btn, .fab, .page-btn, .part-btn, .copy-btn, .adm-btn, .editor-btn, .quick-item');
            if (!btn) return;
            if (btn.dataset.rippleDisabled) return;
    ...
    function init() {
        try {
            ...
            console.warn('Enhancement init error (non-critical):', e);
        }
    }
    ...
    // Expose utils globally for other scripts to use (optional)
    window.libUtils = utils;

})();

/* =====================================================================
   END OF PREMIUM ENHANCEMENTS
   All original forum-specific variables and functions remain unchanged:
   - servImgAccount, servImgId, servImgF, servImgTB, servImgSL, servImgMode
   - iframeSrc, SCE_TopicID, illiwebDomain, servimgDomain
   - INTRANET, quick_reply
   - toggleDarkMode, enforceGroupIcons, switchView, showToast
   - openModal, closeModal, updateServimgTokens
   - initUserSession, loadAnnouncements, loadPremiumCategories
   - loadForumData, loadLatestTopics, loadDiscoverActivity
   - openTopic, applyLuffyAddons, backToList
   - solveRecaptcha, handleSilentRequest, appendGlobalTokens
   - preparePostModal, submitTopic, submitReply, previewTopic, previewReply
   - prepareEdit, previewEdit, submitEdit, quotePost
   - silentAdminAction, openActionModal, openAuthModal
   - processAuthHTML, performLogout, loadSettingsPage
   - routeUrl, buildSidebar, startApp, buildFooterStats
   - executeSearchAndFilter (original + library-specific)
   - All other functions defined in the original script
   ===================================================================== */
